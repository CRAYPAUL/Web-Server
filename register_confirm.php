...
if($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	$repassword = protect_data($_POST['password_confirm']);

	if (!empty($username) && !empty($password) && !empty($repassword)) {
	    if($password == $repassword) {
			if(register_validate($db, $username) == TRUE) {
				$password_hash = password_hash($password, PASSWORD_ARGON2ID);
				if(password_verify($password, $password_hash)) {
					if(register_user($db, $username, $password_hash) == TRUE) {
						echo "<script> window.location.replace('register_success.html') </script>";
					}
					else {
						echo "<script> window.location.replace('https://craypaul.com/') </script>";
					}
				}
				else {
					echo "<script type='text/javascript'>alert('Cannot verify password.');</script>";
					echo "<script> window.location.replace('https://craypaul.com/') </script>";
				}
			}
			else {
